...
try:
    ...
    if STRIPE_SECRET_KEY:
        ...
        print("⚠ Stripe keys not configured - payment features disabled")
except ImportError:
    print("⚠ Stripe module not installed - payment features disabled")
    stripe = None

# Stripe Price IDs (configure these in your Stripe Dashboard)
STRIPE_PRICES = {
    'pro': os.getenv('STRIPE_PRICE_PRO', ''),  # Monthly Pro plan price ID
    'premium': os.getenv('STRIPE_PRICE_PREMIUM', '')  # Monthly Premium plan price ID
}
...
def upgrade_subscription():
    ...
    try:
        ...
        if plan == 'basic':
            return jsonify({
                'success': False,
                'error': 'Basic plan is free, no payment required'
            }), 400
        
        # Check if Stripe is configured
        if not stripe or not STRIPE_SECRET_KEY:
            # Fallback to demo mode if Stripe not configured
            print("⚠️ Stripe not configured, using demo mode")
            ...
            return jsonify({
                'success': True,
                'message': f'Demo mode: Upgraded to {plan.upper()} plan!',
                'plan': plan,
                'redirect': '/dashboard'
            })
        ...
        try:
            ...
            if hasattr(current_user, 'stripe_customer_id') and current_user.stripe_customer_id:
                ...
                # Save customer ID (you may need to add this field to User model)
            ...
            session = stripe.checkout.Session.create(
                ...
                cancel_url=f"{request.host_url}plans?cancelled=true",
                metadata={
    ...
    try:
        if stripe and STRIPE_SECRET_KEY:
            ...
            if session.payment_status == 'paid':
                ...
                flash(f'Successfully subscribed to {plan.upper()} plan!', 'success')
            else:
                flash('Payment not completed', 'error')
        else:
            flash('Payment verification unavailable', 'warning')
...
def stripe_webhook():
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        if user_id and plan:
            try:
                ...
                if user:
                    ...
                    user.subscription_start = datetime.now(UTC)
                    from datetime import timedelta
                    user.subscription_end = datetime.now(UTC) + timedelta(days=30)
                    db.session.commit()
                    print(f"✓ Subscription activated for user {user.username}: {plan}")
            except Exception as e:
        ...
        # (You would need stripe_customer_id field in User model for this)
        print(f"ℹ️ Subscription cancelled for customer {customer_id}")
